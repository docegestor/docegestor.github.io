...
<section class="card"><h2>Ingredientes</h2><ul>{ingredients}</ul></section><section><h2>Modo de preparo</h2><ol>{steps}</ol></section>{f'<section class="card"><h2>Dicas</h2><ul>{tips}</ul></section>' if tips else ''}{source}<p class="meta">Publicado em {published}</p></main>{footer_html()}</body></html>'''
...
def update_index(registry: list[dict[str, Any]]) -> None:
    ...
    page = f'''<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Receitas de doces e bolos | DoceGestor</title><meta name="description" content="Receitas de doces e bolos para preparar, testar e organizar sua produção."><link rel="canonical" href="{BASE_URL}/receitas/">{head_extra()}<style>body{{margin:0;background:#fffaf8;color:#3b1f1b;font:16px/1.7 Arial,sans-serif}}main{{max-width:1080px;margin:auto;padding:24px}}a{{color:#d9553c}}h1{{font:700 44px Georgia,serif}}.intro{{font-size:20px;color:#654a45;max-width:720px}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;margin-top:32px}}.card{{background:white;border:1px solid #f2d9d1;border-radius:16px;padding:22px;box-shadow:0 8px 22px #3b1f1b0d}}.card h2{{font:700 25px Georgia,serif}}.tag{{color:#d9553c;text-transform:uppercase;font-size:13px;font-weight:bold;letter-spacing:.08em}}</style></head><body>{nav_html()}<main><h1>Receitas de doces e bolos</h1><p class="intro">Uma área estática para receitas autorais, revisadas e organizadas para a rotina de confeitaria.</p><div class="grid">{cards}</div></main>{footer_html()}</body></html>'''
